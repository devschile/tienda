import { X, Loader2, MapPin, Mail, Package } from 'lucide-react';
...
interface Order {
  ...
  shipping_region: string | null; shipping_zip: string | null;
  mp_payment_id: string | null; wants_newsletter: boolean;
...
export const STATUS_CONFIG: Record<string, { label: string; color: string; dot: string }> = {
  ...
  pending_transfer: { label: 'Transferencia',color: 'bg-blue-100 text-blue-700',    dot: 'bg-blue-500'  },
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-lg z-50 bg-white shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 shrink-0">
          <div>
            ...
            {order && <p className="text-xs text-slate-400 mt-0.5 font-mono">{order.id.substring(0, 8)}…</p>}
          </div>
        ...
        {loading ? (
          ...
          <div className="flex-1 flex items-center justify-center text-sm text-slate-400">Orden no encontrada</div>
        ) : (
          <div className="flex-1 overflow-y-auto">
                    ...
                    {nextStatuses.map(s => {
                      ...
                      return confirm === s ? (
                        <div key={s} className="flex items-center gap-1.5">
                          ...
                          <button onClick={() => handleStatusChange(s)} disabled={saving}
                        ...
                        <button key={s} onClick={() => setConfirm(s)}
            ...
            <div className="px-6 py-5 space-y-6">
              {/* Cliente */}
              <section>
                <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Cliente</h3>
                    ...
                    return (
                      <div key={i} className="flex items-start gap-3 py-2.5 border-b border-slate-100 last:border-0">
                        ...
                        <div className="text-right shrink-0">
                          <p className="text-sm font-semibold text-slate-700">{formatCLP(item.subtotal)}</p>
                          {hasDiscount && (
                            <p className="text-xs text-slate-400 line-through">
                              {formatCLP((item.original_unit_price ?? item.unit_price) * item.quantity)}
                            </p>
                          )}
                        </div>
                      </div>
              ...
              <div className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-3">
                ...
                <span className="text-xl font-bold text-slate-800">{formatCLP(order.total_amount)}</span>
              </div>
